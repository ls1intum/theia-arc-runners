...
2. **Helm** (v3.8+)
3. **cert-manager** installed in the cluster (for automatic Squid CA generation)
...
| `buildCaches.squid.caSecretName` | `squid-ca-cert` | Name of Squid CA secret (auto-generated by cert-manager) |
...
Most common cause: Squid in `CrashLoopBackOff` while waiting for the cert-manager `Certificate` to be issued. Verify:
